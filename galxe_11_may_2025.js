...
// ✅ NEW: Click "Continue to Access" button if present
setTimeout(function clickContinueButton() {
  const continueBtn = Array.from(document.querySelectorAll('div.w-full.px-7.bg-white.rounded-8.text-black.text-center.text-size-16.font-semibold.cursor-pointer'))
    .find(el => el.textContent.trim() === 'Continue to Access');
  if (continueBtn) {
    clickElement(continueBtn);
    console.log('"Continue to Access" button clicked');
  } else {
    console.log('"Continue to Access" button not found');
  }
}, 3000);
